    ...
    if request.method == 'POST':
        ...
        try:
            if pilot.is_available:
                ...
                flash(
                    f"Pilot {pilot.firstname}, {pilot.lastname} has been assigned to Flight {schedule.flight.code} scheduled for {schedule.origin} to {schedule.destination}",
                    'success')
        ...
        flash(
            f"Could not assign pilot {pilot.firstname}, {pilot.lastname} to flight {schedule.flight.code}. Please ensure the Pilot is available.",
            'danger')
        return redirect(url_for('users.all_pilots'))
